...
export namespace StockOutVoucherModel {
  // Stock Out Request
  export const stockOutRequest = t.Object({
    ...
    // start and end serial numbers (digits only suffix)
    startSerial: t.String({ minLength: 1 }),
    endSerial: t.String({ minLength: 1 }),

    // Required to reconstruct voucher serials (YYMMDD)
  ...
  export const updateStockOutBody = t.Object({
    ...
    stationId: t.Optional(t.String({ format: "uuid" })),
    note: t.Optional(t.String({ maxLength: 500 })),
    // startSerial and endSerial updates generally discouraged or require complex logic.
    // We allow metadata updates.
    notaDinas: t.Optional(t.String()),
    bast: t.Optional(t.String()),
  });
