...
function MatchBadge({
  ...
  if (
    matchDetails &&
    (matchDetails.serialMatch ||
      matchDetails.nikMatch ||
...
export default function ReconciliationDetailPage() {
  ...
  const fetchBatchDetail = useCallback(async () => {
    ...
    try {
      ...
      router.push("/dashboard/superadmin/reconciliation");
    } finally {
      setLoading(false);
    }
  ...
  const filteredRecords = records.filter((rec) => {
    ...
    return (
      ...
      (rec.whoosh.serialNumber &&
        rec.whoosh.serialNumber.toLowerCase().includes(query)) ||
      (rec.fwc?.memberName &&
        rec.fwc.memberName.toLowerCase().includes(query)) ||
      (rec.fwc?.serialNumber &&
        rec.fwc.serialNumber.toLowerCase().includes(query))
    );
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex items-start gap-4">
          <button
            onClick={() => router.push("/dashboard/superadmin/reconciliation")}
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <p className="text-sm text-gray-600">
          Showing {filteredRecords.length}{matchFilter === "all" && fwcOnlyRecords.length > 0 && ` + ${fwcOnlyRecords.length} FWC-only`} of{" "}
        ...
        <table className="w-full min-w-[1400px] text-sm">
          <thead className="border-b bg-gray-50">
            <tr>
              ...
              {/* Whoosh Data */}
              <th
                className="border-l-2 border-blue-200 bg-blue-50 px-3 py-3 text-left font-medium text-blue-800"
                colSpan={3}
              >
                📊 Data Whoosh (Excel)
              </th>
              {/* FWC Data */}
              <th
                className="border-l-2 border-green-200 bg-green-50 px-3 py-3 text-left font-medium text-green-800"
                colSpan={4}
              >
                🗃️ Data FWC (Database)
              </th>
            ...
            <tr className="border-b bg-gray-50">
              ...
              <th className="px-3 py-2"></th>
              {/* Whoosh columns */}
              <th className="border-l-2 border-blue-200 bg-blue-50/50 px-3 py-2 text-left text-xs font-medium text-blue-700">
                Serial
              </th>
              <th className="bg-blue-50/50 px-3 py-2 text-left text-xs font-medium text-blue-700">
                NIK
              </th>
              <th className="bg-blue-50/50 px-3 py-2 text-center text-xs font-medium text-blue-700">
                Tanggal
              </th>
              {/* FWC columns */}
              <th className="border-l-2 border-green-200 bg-green-50/50 px-3 py-2 text-left text-xs font-medium text-green-700">
                Serial
              </th>
              <th className="bg-green-50/50 px-3 py-2 text-left text-xs font-medium text-green-700">
                Member
              </th>
              <th className="bg-green-50/50 px-3 py-2 text-left text-xs font-medium text-green-700">
                Station
              </th>
              <th className="bg-green-50/50 px-3 py-2 text-center text-xs font-medium text-green-700">
                Redeem Date
              </th>
            ...
            {loading ? (
              ...
              filteredRecords.map((record, index) => {
                ...
                const hasPartialMatch =
                  !record.isMatched &&
                  record.matchDetails &&
                  (record.matchDetails.serialMatch ||
                    record.matchDetails.nikMatch ||
                    record.matchDetails.dateMatch);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="bg-blue-50/20 px-3 py-3 text-center text-gray-600">
                      {new Date(record.whoosh.ticketingDate).toLocaleDateString(
                        "id-ID",
                        {
                          day: "2-digit",
                          month: "short",
                    ...
                    <td className="bg-green-50/20 px-3 py-3 text-center text-gray-600">
                      {fwcData?.redeemDate ? (
                        ...
                        >
                          {new Date(fwcData.redeemDate).toLocaleDateString(
                            "id-ID",
                            {
                              day: "2-digit",
                              month: "short",
