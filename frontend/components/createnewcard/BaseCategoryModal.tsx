...
export default function BaseCategoryModal({
  ...
  const fetchRecommendedCode = async () => {
    ...
    try {
      const code = await service.getCategoryRecommend();
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await service.createCategory({ categoryCode, categoryName, description });
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={categoryCode}
                onChange={(e) =>
                  setCategoryCode(e.target.value.replace(/\D/g, "").slice(0, 1))
                }
