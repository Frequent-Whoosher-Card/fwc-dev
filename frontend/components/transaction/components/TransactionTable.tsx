...
import { ChevronLeft, ChevronRight, AlertTriangle, CheckCircle, Eye } from "lucide-react";
...
export interface TransactionItem {
  ...
  // FWC & Voucher (Common)
  card: {
    id: string; // Add ID for FWC
    serialNumber: string;
    status: string; // Add status for FWC
    expiredDate: string | null; // Add for FWC
    quotaTicket: number; // Add for FWC
    cardProduct: {
      totalQuota: number;
  ...
  bulkPurchaseItemsCount?: number;
  firstSerialNumber?: string | null;
...
export default function TransactionTable({
  ...
  canEdit = false,
  canDelete = false,
  programType, // Used to toggle columns
}: Props) {
  /* ======================
     DELETE STATE
  ====================== */
  ...
  const [selectedVoucherTransaction, setSelectedVoucherTransaction] = useState<TransactionItem | null>(null);
  ...
  /* ======================
     DELETE HANDLERS
  ====================== */
  const handleOpenDelete = (transaction: TransactionItem) => {
  ...
  /* ======================
     VOUCHER HANDLERS
  ====================== */
  const handleOpenVoucherModal = async (transaction: TransactionItem) => {
  ...
  // Fetch voucher items when page changes in modal
  useEffect(() => {
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="w-full text-xs border-collapse">
          <thead className="bg-gray-100">
            <tr className="text-[11px] font-semibold text-gray-600">
              ...
              {programType === "VOUCHER" && <th className="px-4 py-3 text-left">Perusahaan</th>}
              ...
              {programType === "FWC" ? (
                  ...
                  <>
                    ...
                    <th className="px-4 py-3 text-left">Range Serial Number</th>
                  </>
              )}
              ...
              {/* Price Columns */}
              {programType === "VOUCHER" && <th className="px-4 py-3 text-right">Subtotal</th>}
              {programType === "VOUCHER" && <th className="px-4 py-3 text-right">Discount</th>}
              <th className="px-4 py-3 text-right">{programType === 'FWC' ? 'FWC Price' : 'Voucher Price'}</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-center gap-2">
                        ...
                        {/* 
                         // Note: Delete button is commented out in original code too, keeping as is but logic is here
                         {canDelete && (
                            <button onClick={() => handleOpenDelete(item)} ... > Hapus </button>
                         )}
                        */}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-[420px] rounded-2xl bg-white p-6 shadow-xl">
            ...
            <h2 className="text-center text-lg font-semibold text-gray-800">
              {programType === "VOUCHER" ? "Hapus Transaksi Voucher" : "Delete Transaction"}
            </h2>
              ...
              {programType === "FWC" ? (
                  ...
                  <>
                    <div className="mt-1 flex justify-between gap-3">
                        ...
                        <span className="max-w-[240px] truncate font-medium text-gray-800">
                        {selectedTransaction?.bulkPurchaseItemsCount ?? selectedTransaction?.bulkPurchaseItems?.length ?? 0} items
                        </span>
                    </div>
                     <div className="mt-1 flex justify-between gap-3">
      ...
      {/* VOUCHER LIST MODAL (Voucher Only) */}
          ...
          <div className="w-[90vw] max-w-4xl max-h-[90vh] rounded-xl bg-white shadow-xl flex flex-col">
            ...
            <div className="flex items-center justify-between p-6 border-b">
              ...
              >
                ...
                {/* Reusing existing icon or X if imported. Using Eye as placeholder or X if available. X is not imported in this scope but View had X. 
                    Let's use a simple text or check imports.
                    Checking imports... X is not imported. I will add X to imports.
                */}
              </button>
            </div>
            
            {/* Content ... (Same as original Voucher table logic) */}
